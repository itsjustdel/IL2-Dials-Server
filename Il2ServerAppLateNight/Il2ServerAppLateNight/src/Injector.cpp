...
{
	toCave = (LPVOID)((uintptr_t)(toCave)+0xC8);
	size_t bytesWritten = 0;
...
bool CaveTurnBall(HANDLE hProcess, uintptr_t src, LPVOID toCave)
{
	...
	//cave in RSE dll already has some cockpit instruments stuff in it so we will put our code after it
	//add to cave,( uintptr_t for addition)
	toCave = (LPVOID)((uintptr_t)(toCave)+0x65);//65 is where we got to with other functions
	//cave - where we put our own code alongside the original
	size_t bytesWritten = 0;
...
bool CaveEngineModification(HANDLE hProcess, uintptr_t src, LPVOID toCave)
{
	...
	//cave in RSE dll already has some cockpit instruments stuff in it so we will put our code after it
	//add to cave,( uintptr_t for addition)
	toCave = (LPVOID)((uintptr_t)(toCave)+0xC8);//C8 is where we got to with other functions
	//cave - where we put our own code alongside the original
	size_t bytesWritten = 0;
	...
	BYTE rbxToMem[7] = { 0x48, 0x89, 0x15, 0xCB, 0x00, 0x00, 0x00 }; 
